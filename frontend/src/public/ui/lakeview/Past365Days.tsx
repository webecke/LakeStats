...
const Past365Days: React.FC<{ lakeSettings: LakeSystemSettings; todayLevel: number }> = ({
    lakeSettings,
    todayLevel,
}) => {
                                    ...
                                    tooltip: {
                                        callbacks: {
                                            title: function (context) {
                                                const dataPoint = context[0].raw as {
                                                    x: string;
                                                    y: number;
                                                };
                                            ...
                                            label: function (context) {
                                                ...
                                                const {
                                                    feet: highlightedFeet,
                                                    inches: highlightedInches,
                                                } = getFeetAndInchesWithFraction(highlightedValue);
                                                const difference = highlightedValue - todayLevel;
                                                const {
                                                    feet: diffFeet,
                                                    inches: diffInches,
                                                    fraction: diffFraction,
                                                } = getFeetAndInchesWithFraction(difference);
                                                const diffText =
                                                    (difference >= 0 ? "+" : "-") +
                                                    (diffFeet > 0
                                                        ? `${diffFeet}ft ${diffInches}in`
